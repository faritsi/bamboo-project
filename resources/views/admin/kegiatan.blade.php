      ...
      <form action="{{ route('kegiatan.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
          ...
          <div class="thumbnail">
            ...
            <input type="file" id="thumbnail" name="image[]" onchange="previewImage(this, 'thumbnail-preview')" multiple>
            @if ($errors->has('image'))
...
<div id="editModal-{{ $i->id }}" class="modal">  
    <div class="modal-content">
      <span class="close">&times;</span>
      <form action="{{ route('kegiatan.update', $i->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
          <div id="head-modul">
          ...
          <div class="thumbnail">
            <img id="edit-thumbnail-preview-{{ $i->id }}" src="{{ $i->image_path ? asset('/storage/' . $i->image_path) : 'https://via.placeholder.com/100' }}" alt="Thumbnail" style="display: block; margin-bottom: 10px; max-width: 100px;">
            <input type="file" id="edit-thumbnail-{{ $i->id }}" name="image" onchange="previewImage(this, 'edit-thumbnail-preview-{{ $i->id }}')">
            @if ($errors->has('image'))
                <p class="alert alert-danger">{{ $errors->first('image') }}</p>
            @endif
        </div>
...
<div id="deleteModal-{{ $i->id }}" class="modal">
    <div class="modal-content">
      ...
      <form action="{{ route('kegiatan.destroy', $i->id) }}" method="POST">
        @csrf
        @method('DELETE')
        <div class="form-group">
    ...
    <div class="card-photo">
        ...
        <div class="photo-card">
            ...
            <div class="photo-overlay">
                <button class="btn-edit" data-id="{{ $i->id }}" data-toggle="modal" data-target="#editModal-{{ $i->id }}">
                <!-- <button class="btn-edit"> -->
                    <span class="material-symbols-outlined">
                        edit
                    </span> 
                </button>
                <button class="btn-delete" data-id="{{ $i->id }}" data-toggle="modal" data-target="#deleteModal-{{ $i->id }}">
                    <span class="material-symbols-outlined">
                        delete
                    </span> 
                </button>
...
<div id="video-container">
    ...
    <div class="wrapper-video">
        ...
        <div class="video-overlay">
            <button class="btn-edit-video" data-id="{{ $v->id }}" data-toggle="modal" data-target="#editModal-{{ $v->id }}">
                <span class="material-symbols-outlined">
                    edit
                </span> 
            </button>
            <button class="btn-delete-video" data-id="{{ $v->id }}" data-toggle="modal" data-target="#deleteModal-{{ $v->id }}">
                <span class="material-symbols-outlined">
                    delete
                </span> 
            </button>
        ...
        @csrf
          ...
          {{-- <div class="thumbnail">
            ...
            <div class="form-group">
                ...
                <textarea id="yt-link" name="video_path"></textarea>
...
<div id="editModalVideo-{{ $v->id }}" class="modal">  
    <div class="modal-content">
      <span class="close">&times;</span>
      <form action="{{ route('kegiatan.update', $v->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
          <div id="head-modul">
            ...
            <div class="form-group">
                <label for="image_produk">Link Youtube</label>
                <textarea id="yt-link" name="video_path">{{$v->video_path}}</textarea>
            </div>
        {{-- </div> --}}
        <div class="form-group">
            <button type="submit" class="submit-btn">Update</button>
...
<div id="deleteModalVideo-{{ $v->id }}" class="modal">
    <div class="modal-content">
      ...
      <form action="{{ route('kegiatan.destroy', $v->id) }}" method="POST">
        @csrf
        @method('DELETE')
        <div class="form-group">
            <button type="submit" class="submit-btn">Hapus</button>
